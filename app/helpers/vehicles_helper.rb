...
module VehiclesHelper
  ...
  def last_a_service_badge(vehicle)
    unless @vehicle.requests.where(a_service: true).empty?
      link_to "(#{@vehicle.requests.where(a_service: true).last.completed_date.strftime('%D')}) | View", request_path(@vehicle.requests.where(a_service: true).last), class: "badge badge-danger"
    else
      'None'
    end
  end
  ...
  def last_air_filter_service_badge(vehicle)
    unless @vehicle.requests.where(air_filter_service: true).empty?
      link_to "(#{@vehicle.requests.where(air_filter_service: true).last.completed_date.strftime('%D')}) | View", request_path(@vehicle.requests.where(air_filter_service: true).last), class: "badge badge-danger"
    else
      'None'
    end
  end
